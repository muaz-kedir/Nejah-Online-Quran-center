      ...
      return (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center mt-6">
          <div className="bg-muted/50 p-4 rounded-2xl border">
            ...
            <p className="font-bold text-foreground mt-1 text-sm">{overview.currentTopic?.label || 'Not Started'}</p>
          </div>
          <div className="bg-muted/50 p-4 rounded-2xl border">
            <p className="text-xs text-foreground font-bold uppercase">Progress</p>
            <p className="font-bold text-foreground mt-1">{p.completed || 0} / {p.total || 0}</p>
          </div>
          <div className="bg-muted/50 p-4 rounded-2xl border">
            <p className="text-xs text-foreground font-bold uppercase">Remaining</p>
            <p className="font-bold text-foreground mt-1">{p.remaining || 0} Topics</p>
          </div>
          <div className="bg-muted/50 p-4 rounded-2xl border">
            <p className="text-xs text-foreground font-bold uppercase">Last Lesson</p>
            <p className="font-bold text-foreground mt-1 text-sm">{lastLog?.date ? new Date(lastLog.date).toLocaleDateString() : '—'}</p>
          </div>
        </div>
      );
    }
      ...
      return (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center mt-6">
          <div className="bg-muted/50 p-4 rounded-2xl border">
            ...
            <p className="font-bold text-foreground mt-1 text-sm">{overview.lastPosition?.lastStudiedSurah || 'Not Started'}</p>
          </div>
          <div className="bg-muted/50 p-4 rounded-2xl border">
            <p className="text-xs text-foreground font-bold uppercase">Current Ayah</p>
            <p className="font-bold text-foreground mt-1">{overview.lastPosition?.lastStudiedAyah || 0}</p>
          </div>
          <div className="bg-muted/50 p-4 rounded-2xl border">
            <p className="text-xs text-foreground font-bold uppercase">Completed Surahs</p>
            <p className="font-bold text-foreground mt-1">{overview.progress?.surahsCount || 0}</p>
          </div>
          <div className="bg-muted/50 p-4 rounded-2xl border">
            <p className="text-xs text-foreground font-bold uppercase">Completed Ayahs</p>
            <p className="font-bold text-foreground mt-1">{overview.progress?.ayahsCount || 0}</p>
          </div>
        </div>
      );
    }
      ...
      return (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center mt-6">
          ...
          <div className="bg-muted/50 p-4 rounded-2xl border">
            ...
            <p className="font-bold text-foreground mt-1">{p.completed || 0}</p>
          </div>
          <div className="bg-muted/50 p-4 rounded-2xl border">
            <p className="text-xs text-foreground font-bold uppercase">Remaining Rules</p>
            <p className="font-bold text-foreground mt-1">{p.remaining || 0}</p>
          </div>
          <div className="bg-muted/50 p-4 rounded-2xl border">
            <p className="text-xs text-foreground font-bold uppercase">Last Lesson</p>
            <p className="font-bold text-foreground mt-1 text-sm">{lastLog?.date ? new Date(lastLog.date).toLocaleDateString() : '—'}</p>
      ...
      return (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center mt-6">
          ...
          <div className="bg-muted/50 p-4 rounded-2xl border">
            ...
            <p className="font-bold text-foreground mt-1">{Math.floor((overview.progress?.surahsCount || 0) / 4)}</p>
          </div>
          <div className="bg-muted/50 p-4 rounded-2xl border">
            <p className="text-xs text-foreground font-bold uppercase">Revision Progress</p>
            <p className="font-bold text-foreground mt-1">{overview.progress?.progressPercentage || 0}%</p>
          </div>
        </div>
      );
    }
    ...
    <StudentPortalLayout activePath={studentPaths.progress}>
      <main className="flex-1 px-4 sm:px-6 lg:px-10 py-6 lg:py-10 max-w-5xl mx-auto print:p-0 print:max-w-none">
        ...
        <div className="hidden print:block text-center mb-8 border-b pb-6">
          <h1 className="text-3xl font-extrabold text-foreground font-serif">Nejah Quran Institute</h1>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10 print:hidden">
          <div>
            ...
            <h1 className="text-4xl font-extrabold text-foreground font-serif">My Progress</h1>
          </div>
          <Button onClick={handlePrint} variant="outline" className="gap-2 font-bold rounded-xl border-nejah-blue/20 text-foreground hover:bg-nejah-blue/10">
        ...
        <div className="bg-card rounded-[32px] p-8 border mb-8 print:shadow-none print:border-gray-200">
          <div className="flex justify-between items-start mb-6">
            ...
            <div className="text-right">
              <Badge className="bg-nejah-blue/10 text-foreground border-none px-3 py-1 mb-2">
                <Award className="h-4 w-4 mr-1.5" /> {overview.progress?.rank || overview.rank || 'Beginner'}
              </Badge>
              <p className="text-2xl font-black text-foreground">{data.percentage}%</p>
            </div>
          </div>
              ...
              {data.evaluations.map((ev: any) => (
                <div key={ev.id} className="bg-muted/30 rounded-2xl p-6 border">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h4 className="font-bold text-foreground text-lg">{ev.evaluationType}</h4>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                ...
                {data.timeline.slice(0, 10).map((item: any, i: number) => (
                  <div key={i} className="relative group">
                    ...
                    <div className="bg-muted/50 p-4 rounded-2xl border hover:border-nejah-blue/30 transition-colors cursor-pointer print:border-none print:p-0 print:bg-transparent" onClick={() => setSelectedDetails(item)}>
                      ...
                      <p className="font-bold text-foreground mt-2">Lesson completed</p>
                      ...
                      <div className="flex items-center justify-between mt-3 print:hidden">
                        ...
                        <Button variant="ghost" size="sm" className="h-6 text-xs px-2 text-foreground">View Details <ChevronRight className="w-3 h-3 ml-1" /></Button>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="space-y-8">
            ...
            <div className="bg-card rounded-[32px] p-8 border">
              ...
              <h3 className="text-xl font-bold text-foreground mb-6 hidden print:block">Attendance History</h3>

              {data.attendanceStats && (
                <div className="grid grid-cols-4 gap-2 text-center mb-6 p-4 bg-muted/30 rounded-2xl border">
                  <div><p className="text-2xl font-black text-foreground">{data.attendanceStats.present}</p><p className="text-[10px] font-bold text-foreground uppercase tracking-wider">Present</p></div>
                ...
                <div className="space-y-3">
                  {filteredAttendance.slice(0, 10).map((ah: any) => (
                    <div key={ah.id} className="flex items-center justify-between p-3 rounded-xl border bg-card text-sm">
                      <div>
                        <p className="font-bold text-foreground">{new Date(ah.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                  ...
                  {data.teacherFeedback.slice(0, 5).map((f: any) => (
                    <div key={f.id} className="border-b pb-4 last:border-0 last:pb-0">
                      <div className="flex justify-between items-center mb-2">
                        <p className="font-bold text-sm text-foreground flex items-center gap-1.5"><User className="w-3 h-3"/> {f.teacherName}</p>
      ...
      <Dialog open={!!selectedDetails} onOpenChange={(open) => !open && setSelectedDetails(null)}>
        <DialogContent className="sm:max-w-[500px] rounded-[32px] p-8">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-foreground font-serif">Lesson Details</DialogTitle>
            <DialogDescription>
          ...
          <div className="mt-4 space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-muted/50 p-4 rounded-2xl border">
                ...
                <p className="font-bold text-foreground">{selectedDetails?.courseLevel}</p>
              </div>
              <div className="bg-muted/50 p-4 rounded-2xl border">
                <p className="text-xs font-bold text-foreground uppercase mb-1">Teacher</p>
                <p className="font-bold text-foreground">{selectedDetails?.teacherName || 'Assigned Teacher'}</p>
              </div>
            </div>
