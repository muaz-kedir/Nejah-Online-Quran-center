...
function StudentPaymentsPage() {
  ...
  const convertCurrency = async () => {
    try {
      const res = await fetch(`${API_BASE}/currency/convert?from=ETB&to=${convertTarget}&amount=1`, {
        headers: authHeaders(),
      });
  ...
  const recordPayment = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${API_BASE}/finance/student-payments/${detail.id}/transactions`, {
        method: 'POST',
        headers: authHeaders(),
  ...
  const markStatus = async (status: string) => {
    if (!detail) return;
    try {
      const res = await fetch(`${API_BASE}/finance/student-payments/${detail.id}`, {
        method: 'PATCH',
        headers: authHeaders(),
