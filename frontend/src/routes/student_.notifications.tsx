...
function matchesTypeFilter(channel: string, typeFilter: string): boolean {
  if (!typeFilter) return true;
  switch (typeFilter) {
    case "homework":
      return channel.startsWith("HOMEWORK");
    case "classes":
      return ["MEETING_STARTED", "MEETING_ENDED", "CLASS_REMINDER", "SESSION_CANCELLED", "SCHEDULE_CHANGED", "CLASS_ALERT"].includes(channel);
    case "evaluations":
      return ["EVALUATION_PUBLISHED", "EXAM_RESULTS", "TEACHER_FEEDBACK"].includes(channel);
    case "attendance":
      return ["ATTENDANCE_MARKED", "ATTENDANCE_CORRECTED"].includes(channel);
    case "resources":
      return channel.startsWith("RESOURCE");
    case "announcements":
      return ["SYSTEM_ANNOUNCEMENT", "FEATURE_UPDATE"].includes(channel);
    case "schedule":
      return ["SCHEDULE_CHANGED", "TEMP_REPLACEMENT", "CLASS_ALERT"].includes(channel);
    case "system":
      return ["SYSTEM_ALERT", "MAINTENANCE", "PAYMENT_REMINDER", "PAYMENT_RECEIVED", "PAYMENT_OVERDUE"].includes(channel);
    default:
      return true;
  }
}
