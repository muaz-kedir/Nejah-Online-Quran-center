      ...
      <main className="flex-1 px-10 py-10 max-w-4xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
          <div>
            ...
            <h1 className="text-4xl font-extrabold text-emerald-950 font-serif">Notifications</h1>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => setFilter(filter === 'all' ? 'unread' : 'all')}>
              {filter === 'all' ? 'Unread only' : 'Show all'}
            </Button>
            <Button size="sm" className="bg-emerald-700" onClick={markAllRead} disabled={unread === 0}>
          ...
          <div className="space-y-3">
            {filtered.map((n) => (
              <button
                ...
                className={cn(
                  'w-full text-left bg-white rounded-2xl p-5 border transition-all',
                  n.isRead ? 'border-gray-100 opacity-80' : 'border-emerald-200 shadow-sm',
                )}
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h3 className="font-bold text-emerald-950">{n.title}</h3>
                  ...
                  {!n.isRead && (
                    <Badge className="bg-emerald-100 text-emerald-800 border-none shrink-0">New</Badge>
                  )}
                </div>
              </button>
            ))}
