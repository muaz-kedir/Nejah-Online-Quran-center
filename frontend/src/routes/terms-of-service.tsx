...
export const Route = createFileRoute("/terms-of-service")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Terms of Service for Nejah Online Quran Center" },
    ],
  }),
});
...
function TermsContent() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await getPublishedPage("terms-of-service");
        setPage(data);
      } catch (err) {
        console.warn("Terms of service page not published yet", err);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const title = page.title?.[lang] || page.title?.en || "Terms of Service";
