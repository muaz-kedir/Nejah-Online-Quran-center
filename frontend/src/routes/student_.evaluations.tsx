    ...
    <StudentPortalLayout activePath={studentPaths.evaluations}>
      <main className="flex-1 px-4 sm:px-6 lg:px-10 py-6 lg:py-10 max-w-4xl space-y-8">
        <div>
          ...
          <h1 className="text-4xl font-extrabold text-foreground font-serif">My Exam Evaluations</h1>
        ...
        {evaluations.length === 0 ? (
          <Card className="rounded-[32px] border p-8 text-center bg-card">
            <CardContent className="pt-6">
              ...
              <h3 className="text-lg font-bold text-foreground font-serif">No evaluations recorded yet</h3>
              ...
              <Card key={ev.id} className="rounded-[32px] border overflow-hidden bg-card shadow-sm hover:shadow-md transition-shadow">
                <CardHeader className="bg-muted/30 border-b p-6 flex flex-row flex-wrap items-center justify-between gap-4">
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-3xl font-black text-foreground font-serif">{ev.score}</span>
                ...
                <CardContent className="p-6 md:p-8 space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="md:col-span-2 space-y-2">
                      <h4 className="text-sm font-extrabold text-foreground uppercase tracking-wider">Teacher Comments</h4>
                      <p className="text-sm text-foreground italic bg-muted/20 p-4 rounded-2xl border">&quot;{ev.teacherComments}&quot;</p>
                    </div>

                    {/* Recommendation */}
                    <div className="space-y-2">
                      <h4 className="text-sm font-extrabold text-foreground uppercase tracking-wider">Promotion Readiness</h4>
                  ...
                  {ev.recommendations && (
                    <div className="space-y-2 pt-2">
                      <h4 className="text-sm font-extrabold text-foreground uppercase tracking-wider">Recommendations</h4>
                  ...
                  {ev.criteriaRatings && Object.keys(ev.criteriaRatings).length > 0 && (
                    <div className="space-y-3 pt-2">
                      <h4 className="text-sm font-extrabold text-foreground uppercase tracking-wider border-b pb-1.5">Criteria Breakdown</h4>
...
export const Route = createFileRoute('/student_/evaluations')({
