    ...
    <div className="bg-card rounded-[24px] p-6 border border-border shadow-sm">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {showJoin && (
            <Button size="sm" className="bg-primary" onClick={() => joinClass(cls.meetingLink, data.liveClass?.id)}>
    ...
    <StudentPortalLayout activePath={studentPaths.classes}>
      <main className="flex-1 px-4 sm:px-6 lg:px-10 py-6 lg:py-10 max-w-5xl">
        <div className="mb-10">
          ...
          <h1 className="text-4xl font-extrabold text-nejah-sapphire font-serif">My Classes</h1>
        </div>
        ...
        {data.liveClass && (
          <section className="mb-10">
            ...
            <div className="bg-nejah-sapphire text-white rounded-3xl p-8">
              <h3 className="text-2xl font-bold">{data.liveClass.classTitle}</h3>
              <Button className="mt-4 bg-white text-nejah-sapphire" onClick={() => joinClass(data.liveClass.meetingLink, data.liveClass.id)}>
                Enter Live Class
              </Button>
            </div>
          </section>
        ...
        <section className="mb-10">
          <h2 className="text-lg font-bold text-nejah-sapphire mb-4">Current Class</h2>
          {data.current?.length ? (
            <div className="space-y-4">{data.current.map((c: any) => <ClassCard key={c.id} cls={c} showJoin />)}</div>
          ) : (
            <p className="text-muted-foreground text-sm">No class scheduled for today.</p>
          )}
        </section>

        <section className="mb-10">
          <h2 className="text-lg font-bold text-nejah-sapphire mb-4">Upcoming Classes</h2>
        ...
        <section>
          <h2 className="text-lg font-bold text-nejah-sapphire mb-4">Previous Classes</h2>
          {data.previous?.length ? (
            <div className="space-y-4">
              {data.previous.map((c: any) => (
                <div key={c.id} className="bg-muted rounded-2xl p-5 border">
                  <div className="flex justify-between text-sm">
                    <span className="font-bold text-nejah-sapphire">
                      {c.classDate ? new Date(c.classDate).toLocaleDateString() : '—'}
                    </span>
                    <span className="text-muted-foreground">{c.duration ? `${c.duration} min` : '—'}</span>
                  </div>
                  <p className="mt-2 font-medium">{c.lessonCovered || 'Quran Class'}</p>
                  {c.teacherNotes && <p className="text-sm text-muted-foreground mt-2 italic">"{c.teacherNotes}"</p>}
                  <p className="text-xs text-muted-foreground mt-2">Teacher: {c.teacherName || '—'}</p>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-muted-foreground text-sm">No completed class history yet.</p>
          )}
        </section>
      </main>
    </StudentPortalLayout>
